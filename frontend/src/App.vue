    ...
    <div v-if="!isExpanded" class="layout layout-centered">
      <section class="panel panel-form panel-centered">
        <header class="panel-head">
          ...
          <div>
            <h1>MechResearch-Agent</h1>
            <p>工程技术深度研究智能体 — 多轮检索、知识库增强、报告评估。</p>
          </div>
        </header>
        ...
        <form class="form" @submit.prevent="handleSubmit">
          <label class="field">
            ...
            <textarea
              v-model="form.topic"
              placeholder="例如：CFD 湍流模型在航空发动机叶片冷却中的应用与对比"
              rows="4"
              required
            ></textarea>
          </label>
        ...
        <div class="kb-panel">
          ...
          <div class="kb-upload">
            <label class="kb-upload-btn">
              ...
              上传文档
              <input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.md" @change="handleKBUpload" hidden />
            </label>
            <span class="kb-hint">支持 PDF/Word/Excel/PPT/TXT/MD</span>
          </div>
          <p v-if="kbUploadStatus" class="kb-status" :class="kbUploadStatus.type">
            {{ kbUploadStatus.message }}
          </p>
      ...
      >
        ...
        <div class="tasks-section" v-if="todoTasks.length">
          ...
          <article class="task-detail" v-if="currentTask">
            ...
            <!-- 知识库上下文标记 -->
            <section v-if="currentTask && currentTask.kbContext" class="kb-context-block">
              <h4>本地知识库补充</h4>
              <pre class="block-pre kb-pre">{{ currentTask.kbContext }}</pre>
            </section>
        ...
        <div v-if="evaluation" class="evaluation-block">
          ...
          <div class="eval-score-row">
            <div class="eval-score-circle" :class="evalScoreClass">
              <span class="eval-score-num">{{ evaluation.score }}</span>
              <span class="eval-score-label">分</span>
            </div>
            <div class="eval-checks">
              <div class="eval-check" :class="{ pass: evaluation.has_sources }">
                <span class="eval-icon">{{ evaluation.has_sources ? '✓' : '✗' }}</span>
                来源完整性
              </div>
              <div class="eval-check" :class="{ pass: evaluation.has_tech_stack }">
                <span class="eval-icon">{{ evaluation.has_tech_stack ? '✓' : '✗' }}</span>
                技术栈建议
              </div>
              <div class="eval-check" :class="{ pass: evaluation.has_risks }">
                <span class="eval-icon">{{ evaluation.has_risks ? '✓' : '✗' }}</span>
                风险分析
              </div>
              <div class="eval-check" :class="{ pass: evaluation.has_actionable_steps }">
                <span class="eval-icon">{{ evaluation.has_actionable_steps ? '✓' : '✗' }}</span>
                可执行步骤
              </div>
            </div>
          </div>
          <div v-if="evaluation.suggestions && evaluation.suggestions.length" class="eval-suggestions">
            <h4>改进建议</h4>
            <ul>
              <li v-for="(s, i) in evaluation.suggestions" :key="i">{{ s }}</li>
            </ul>
          </div>
          <p v-if="evaluation.details" class="eval-details">{{ evaluation.details }}</p>
        </div>
      </section>
...
const kbUploadStatus = ref<{ type: string; message: string } | null>(null);
const kbStats = ref<string>("");
...
const canExportPdf = computed(() => {
  const report = reportMarkdown.value.trim();
  return (
    report.length > 0 &&
    report !== "暂无生成的报告" &&
    report !== "报告生成失败，未获得有效内容"
  );
});

const evalScoreClass = computed(() => {
...
async function handleKBUpload(event: Event) {
  const input = event.target as HTMLInputElement;
  const file = input.files?.[0];
  if (!file) return;

  kbUploadStatus.value = { type: "info", message: `正在上传 ${file.name}...` };

  try {
    const result = await uploadToKB(file);
    if (result.status === "ok") {
      kbUploadStatus.value = { type: "success", message: `${file.name} 上传成功` };
      refreshKBStats();
    } else {
      kbUploadStatus.value = { type: "error", message: String(result.message || "上传失败") };
    }
  } catch (err) {
    kbUploadStatus.value = {
      type: "error",
...
async function refreshKBStats() {
  try {
    ...
    if (result.status === "ok") {
      kbStats.value = String(result.result || "");
    } else if (result.status === "unavailable") {
      kbStats.value = "知识库未启用";
    }
  } catch {
    kbStats.value = "";
  }
...
function sanitizePdfFilename(value: string): string {
  ...
  return cleaned.slice(0, 80) || "MechResearch-Agent研究报告";
}
...
async function handleExportPdf() {
  if (!canExportPdf.value) {
    progressLogs.value.push("当前没有可导出的正式报告");
    return;
  }

  exportingPdf.value = true;
  progressLogs.value.push("正在导出 PDF...");

  try {
    const titleBase = form.topic.trim() || "MechResearch-Agent研究报告";
    const blob = await exportReportPdf(titleBase, reportMarkdown.value.trim());
    const filename = `${sanitizePdfFilename(titleBase)}.pdf`;
    downloadPdfBlob(blob, filename);
    progressLogs.value.push(`PDF 已导出：${filename}`);
  } catch (err) {
  ...
  try {
    await runResearchStream(
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "kb_context") {
          const payload = event as Record<string, unknown>;
          const task = findTask(payload.task_id);
          if (task && typeof payload.content === "string") {
            task.kbContext = payload.content;
            progressLogs.value.push(`任务 ${task.title} 获得知识库补充`);
          }
          return;
        }
        ...
        if (event.type === "evaluation") {
          ...
          evaluation.value = {
            ...
            has_sources: Boolean(payload.has_sources),
            has_tech_stack: Boolean(payload.has_tech_stack),
            has_risks: Boolean(payload.has_risks),
            has_actionable_steps: Boolean(payload.has_actionable_steps),
            suggestions: Array.isArray(payload.suggestions)
              ? (payload.suggestions as string[])
              : [],
            details: typeof payload.details === "string" ? payload.details : ""
          };
          progressLogs.value.push(`报告评估完成，得分：${evaluation.value.score}`);
          return;
        }
...
.eval-score-circle.score-high {
  border-color: #22c55e;
  background: rgba(34, 197, 94, 0.08);
  color: #15803d;
}

.eval-score-circle.score-mid {
  border-color: #f59e0b;
  background: rgba(245, 158, 11, 0.08);
  color: #b45309;
}

.eval-score-circle.score-low {
  border-color: #ef4444;
