"""Orchestrator coordinating the MechResearch deep research workflow."""
...
class DeepResearchAgent:
    ...
    def __init__(self, config: Configuration | None = None) -> None:
        ...
        self.kb_service = KnowledgeBaseService(self.config)
        self.evaluator = EvaluatorService(self.llm) if self.config.enable_evaluator else None
    ...
    def run(self, topic: str) -> SummaryStateOutput:
        ...
        evaluation = None
        if self.evaluator and report:
            evaluation = self.evaluator.evaluate(report, state.todo_items)
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        ...
        yield {"type": "status", "message": "初始化工程技术研究流程"}
        ...
        if self.evaluator and report:
            yield {"type": "status", "message": "正在评估报告质量..."}
            try:
                evaluation = self.evaluator.evaluate(report, state.todo_items)
                yield {
                    "type": "evaluation",
    ...
    def _execute_task(
        ...
        *,
        ...
        kb_context = ""
        if self.kb_service.available:
            kb_context = self.kb_service.search(task.query)
            if kb_context:
                task.kb_context = kb_context
                context = f"{context}\n\n[本地知识库补充]\n{kb_context}"
                if emit_stream:
                    yield {
                        "type": "kb_context",
    ...
    def _persist_final_report(self, state: SummaryState, report: str) -> dict[str, Any] | None:
        ...
        note_title = f"工程技术研究报告：{state.research_topic}".strip() or "工程技术研究报告"
        tags = ["deep_research", "mech_research", "report"]
